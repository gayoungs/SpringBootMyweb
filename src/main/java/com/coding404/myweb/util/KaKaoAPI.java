...
public class KaKaoAPI {
	...
	public String getToken(String code) {
		...
		try {
			...
			String req = "grant_type=authorization_code"
					+ "&client_id=0283c760642bb285136809ed8c816403"
	...
	public Map<String, Object> getUser(String token) {
		...
		try {
			...
			conn.addRequestProperty("Authorization", "Bearer " + token); //api를 사용하기 위해 발급받은 토큰값
			...
			if(conn.getResponseCode() == 200) {
				...
				JsonObject properties = element.getAsJsonObject().get("properties").getAsJsonObject(); //JSON오브젝트
